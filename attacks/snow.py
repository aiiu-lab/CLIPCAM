...
class GaussianSmoothing(nn.Module):
    ...
    """
    def __init__(self, kernel_size, sigma, channels=1, gpu_id=0):
        ...
        kernel = kernel.repeat(channels, *[1] * (kernel.dim() - 1)).to(gpu_id)
...
def make_kernels(snow_length_bound=13, blur=True, gpu_id = 0):
    kernels = []
    ...
    for i in range(7):
        ...
        kernel = torch.FloatTensor(k_npy.copy()).view(1,1,k_size,k_size).to(gpu_id)

        if blur:
            blurriness = np.random.uniform(0.41, 0.6)
            gaussian_blur = GaussianSmoothing(int(np.ceil(5 * blurriness)), blurriness, gpu_id=gpu_id).to(gpu_id)
...
def snow_creator(intensities, k, resol, gpu_id):
    flake_grids = []
    k = torch.cat(k, 1).to(gpu_id)
    intensities_pow = torch.pow(intensities, 4).to(gpu_id)
    flake_grids = torch.zeros((intensities.size(0), k.size(1), resol, resol)).to(gpu_id)
