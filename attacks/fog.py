...
def fog_creator(fog_vars, bsize=1, mapsize=256, wibbledecay=1.75, gpu_id = 0):
    assert (mapsize & (mapsize - 1) == 0)
    maparray = torch.from_numpy(np.empty((bsize, mapsize, mapsize), dtype=np.float32)).to(gpu_id)
    maparray[:, 0, 0] = 0
    stepsize = mapsize
    wibble = 100
